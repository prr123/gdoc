...
//	fmt.Printf("args: %d\n", numArgs)
	...
	switch numArgs {
		case 1:
 			...
 			fmt.Printf("%s usage is:\n  %s docId foldername opt [sumary, main, all]\n", cmd[2:], cmd)
        	os.Exit(1)

		case 2:
			fmt.Println("no folder name for ouput file provided!")
...
//		err = gdocHtml.CreGdocHtmlDoc(outfilPath, doc, nil)
	...
	case "all":
		err = gdocMdDom.CreGdocDomAll(outfilPath, inpFilNam, nil)
		if err != nil {
			fmt.Println("error CreMdDomAll -- cannot convert md file: ", err)
			os.Exit(1)
		}
